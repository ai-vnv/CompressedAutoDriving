"""Render one audited F10-L1 SAC lap on real Gym-Duckietown ``small_loop``.

The policy receives only the frozen six-dimensional lane observation.  The
yellow-line clearance displayed in the overlay is evaluation-only data and is
never passed back to SAC.
"""
...
DEFAULT_CONFIG = ROOT / "configs" / "f10_l1_lane_v1.toml"
DEFAULT_ARTIFACT_DIR = ROOT / "artifacts" / "f10_l1"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=DEFAULT_ARTIFACT_DIR / "sac_lane_demo.mp4",
    )
...
def _annotate(
    ...
    *,
    ...
    lines = (
        "F10-L1 SAC | small_loop | COUNTER-CLOCKWISE | real simulator",
...
def render(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.seed not in protocol.seeds.development:
        raise ValueError("proof video seed must belong to the frozen development split")
    ...
    rewards = {name: 0.0 for name in (
        "reward_progress", "reward_lane", "reward_yellow", "reward_comfort",
        "reward_living", "reward_terminal",
    )}
    lateral: list[float] = []
    heading: list[float] = []
