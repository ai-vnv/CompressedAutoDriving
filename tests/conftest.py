...
evaluation ledgers under ``artifacts/`` and hash-checks the frozen study
documents. Those inputs are not distributed with the repository, since the
telemetry, rollout media, and result ledgers run to several gigabytes and are
regenerated from ``configs/`` and ``experiments/``.
...
# Modules that read the evaluation ledgers, the recorded media, or the frozen
# study documents. Established by running the suite against a checkout with
# those paths absent; every entry fails or errors without them.
...
collect_ignore = []
if not ARTIFACTS.is_dir() or not DOCS.is_dir():
    collect_ignore.extend(REQUIRES_ARTIFACTS_OR_DOCS)


def pytest_report_header(config):
    if collect_ignore:
        missing = [p.name for p in (ARTIFACTS, DOCS) if not p.is_dir()]
